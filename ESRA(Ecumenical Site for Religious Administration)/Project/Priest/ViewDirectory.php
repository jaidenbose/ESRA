...
<html lang="en" >
    ...
    <body>
        <h1 align="center" id="heading">Data</h1>
        ...
        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody>
            <?php 
	$selQ="select * from tb_family f inner join tb_unit p on f.unit_id=p.unit_id ";
	$i=0;
	$row=$conn->query($selQ);
	while($data=$row->fetch_assoc())
	{
		$i++;
		$age= date_diff(date_create($data["fam_dob"]), date_create('today'))->y;
	
	?>
     <tr>
      <td><?php echo $i; ?></td>
      <td><?php echo $data["fam_headname"]; ?></td>
      <td><?php echo $data["fam_address"]; ?></td>
      <td><?php echo $data["fam_mail"]; ?></td>
      <td><?php echo $data["fam_occupation"]; ?></td>
      <td><?php echo $data["fam_name"]; ?></td>
      <td><?php echo $data["fam_gender"]; ?></td>
      <td><?php echo $data["unit_name"]; ?></td>
      <td><?php echo $data["fam_dob"]; ?></td>
      <td><?php echo  $age ?></td>
	...
	?> 
                ...
                <?php 
	$selQ="select * from tb_user u inner join tb_unit p on u.unit_id=p.unit_id inner join tb_family f on f.fam_id=u.fam_id";
	$i=0;
	$row=$conn->query($selQ);
	while($data=$row->fetch_assoc())
	{
		$i++;
		$age= date_diff(date_create($data["user_dob"]), date_create('today'))->y;
